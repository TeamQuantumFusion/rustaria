...
use rsa_api::loader::Loader;
use rsa_api::plugin::Plugins;
...
#[structopt(name = "rsa-server", about = "The serverside face of Rustaria")]
struct Opt {
    #[structopt(flatten)]
    inner: rsa_common::opt::Opt,
}
...
fn main() -> Result<()> {
    ...
    let Opt {
        inner: rsa_common::opt::Opt { verbosity, run_dir },
    } = opt;
    rsa_common::init(verbosity)?;
