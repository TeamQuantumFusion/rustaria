...
use crate::{
	...
	table::Table,
	thread::Thread,
	types::{Integer, LightUserData, Number},
...
impl MultiValue {
	...
	#[inline]
	pub fn new() -> MultiValue { MultiValue(Vec::new()) }

	/// Similar to `new` but can return previously used container with allocated capacity.
	#[inline]
	pub(crate) fn new_or_cached(lua: &Lua) -> MultiValue { lua.new_or_cached_multivalue() }
}

impl Default for MultiValue {
	#[inline]
	fn default() -> MultiValue { MultiValue::new() }
}
...
impl<'a, 'lua> IntoIterator for &'a MultiValue {
	...
	#[inline]
	fn into_iter(self) -> Self::IntoIter { (&self.0).iter().rev() }
}
...
impl MultiValue {
	...
	#[inline]
	pub(crate) fn reserve(&mut self, size: usize) { self.0.reserve(size); }

	#[inline]
	pub(crate) fn push_front(&mut self, value: Value) { self.0.push(value); }

	#[inline]
	pub(crate) fn pop_front(&mut self) -> Option<Value> { self.0.pop() }
	...
	#[inline]
	pub fn iter(&self) -> iter::Rev<slice::Iter<Value>> { self.0.iter().rev() }

	#[inline]
	pub(crate) fn drain_all(&mut self) -> iter::Rev<vec::Drain<Value>> { self.0.drain(..).rev() }
