...
use crate::{
	types::MaybeSend, FromLuaMulti, Lua, LuaWeak, MetaMethod, ToLuaMulti, UserData, UserDataMethods,
};
	...
	for GlueUserDataMethods<'a, V, I>
{
	...
	{
		self.methods.add_method(name, move |lua, data, args| {
	...
	{
		self.methods.add_method_mut(name, move |lua, data, args| {
	...
	where
		...
		M: 'static + Send + Fn(&Lua, &V, A) -> eyre::Result<R>,
	{
		self.methods.add_meta_method(meta, move |lua, data, args| {
			method(lua, data.get("this")?, args)
		})
	}
