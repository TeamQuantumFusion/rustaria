...
use std::{
	error::Error as StdError, fmt, fmt::Debug, io::Error as IoError, net::AddrParseError,
	result::Result as StdResult, str::Utf8Error, string::String as StdString, sync::Arc,
};
	...
	pub fn report(self) -> Audit {
		match self {
			Error::CallbackError { traceback, cause } => {
				...
				{
					cause = cause2;
					_full_traceback = Some(traceback2);
				}
...
impl<E: Into<Box<dyn StdError + Send + Sync>>> ExternalError for E {
	fn to_lua_err(self) -> Error { Error::external(self) }
}
...
where
	E: ExternalError,
{
	fn to_lua_err(self) -> Result<T> { self.map_err(|e| e.to_lua_err()) }
}

impl From<AddrParseError> for Error {
	fn from(err: AddrParseError) -> Self { Error::external(err) }
}

impl From<IoError> for Error {
	fn from(err: IoError) -> Self { Error::external(err) }
}

impl From<Utf8Error> for Error {
	fn from(err: Utf8Error) -> Self { Error::external(err) }
}

impl From<Audit> for Error {
	fn from(err: Audit) -> Self { Error::Audit(Arc::new(err)) }
}

impl From<RefError> for Error {
	fn from(err: RefError) -> Self { Error::RefError(err) }
}

#[cfg(feature = "serialize")]
impl serde::ser::Error for Error {
	fn custom<T: fmt::Display>(msg: T) -> Self { Self::SerializeError(msg.to_string()) }
}

#[cfg(feature = "serialize")]
impl serde::de::Error for Error {
	fn custom<T: fmt::Display>(msg: T) -> Self { Self::DeserializeError(msg.to_string()) }
}
