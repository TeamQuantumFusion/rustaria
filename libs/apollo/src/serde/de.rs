use std::{cell::RefCell, convert::TryInto, os::raw::c_void, rc::Rc, string::String as StdString};
...
impl Deserializer {
	...
	pub fn new(value: Value) -> Self { Self::new_with_options(value, Options::default()) }
...
impl<'lua, 'de> serde::Deserializer<'de> for Deserializer {
		...
		V: de::Visitor<'de>,
	{
		match self.value {
			...
			Value::Integer(i) => visitor.visit_i64(i.try_into().expect("cannot convert lua_Integer to i64")),
			#[allow(clippy::useless_conversion)]
...
impl Drop for RecursionGuard {
	fn drop(&mut self) { self.visited.borrow_mut().remove(&self.ptr); }
}
