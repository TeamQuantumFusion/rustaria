...
pub trait AsChunk {
	...
	fn name(&self) -> Option<StdString> { None }

	/// Returns optional chunk [environment]
	///
	/// [environment]: https://www.lua.org/manual/5.4/manual.html#2.2
	fn env(&self, _lua: &Lua) -> Result<Option<Value>> { Ok(None) }

	/// Returns optional chunk mode (text or binary)
	fn mode(&self) -> Option<ChunkMode> { None }
}

impl AsChunk for str {
	fn source(&self) -> IoResult<Cow<[u8]>> { Ok(Cow::Borrowed(self.as_ref())) }
}
...
impl AsChunk for [u8] {
	fn source(&self) -> IoResult<Cow<[u8]>> { Ok(Cow::Borrowed(self)) }
}
...
impl AsChunk for Path {
	fn source(&self) -> IoResult<Cow<[u8]>> { std::fs::read(self).map(Cow::Owned) }

	fn name(&self) -> Option<StdString> { Some(format!("@{}", self.display())) }
}
