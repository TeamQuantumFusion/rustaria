...
use crate::{
	...
	util::{check_stack, get_userdata, StackGuard},
	value::{FromLua, FromLuaMulti, ToLua, ToLuaMulti, Value},
};
...
impl<T: 'static + UserData> UserDataMethods<T> for StaticUserDataMethods<T> {
	...
	#[cfg(feature = "async")]
	fn add_async_meta_callback(&mut self, meta: MetaMethod, callback: AsyncCallback<'static>) {
		...
		M: 'static + MaybeSend + Fn(&Lua, Ref<T>, A) -> anyways::Result<R>,
	{
		Box::new(move |lua, mut args| {
			if let Some(front) = args.pop_front() {
				...
				Err(Audit::new(Error::FromLuaConversionError {
			...
			let fut_res = || {
				if let Some(front) = args.pop_front() {
					...
					unsafe {
						...
						match type_id {
							Some(id) if id == TypeId::of::<T>() => {
								let cell = get_userdata_cell::<T>(lua.state);
								let reference: Ref<T> = cell.get("self")?;
								Ok(method(
									lua,
									reference.clone(),
...
pub(crate) struct StaticUserDataFields<T: 'static + UserData> {
	...
	pub(crate) meta_fields: Vec<(MetaMethod, Box<dyn Fn(&Lua) -> Result<Value> + 'static>)>,
	_type: PhantomData<T>,
}
...
impl<T: 'static + UserData> UserDataFields<T> for StaticUserDataFields<T> {
	...
	fn add_field_method_get_mut<S, R, M>(&mut self, name: &S, mut method: M)
	where
		S: AsRef<[u8]> + ?Sized,
		R: ToLua,
		...
		M: 'static + MaybeSend + Fn(&Lua, Ref<T>, A) -> anyways::Result<()>,
	{
		self.field_setters.push((
			name.as_ref().to_vec(),
			StaticUserDataMethods::box_method(method),
		));
	}
...
#[inline]
unsafe fn get_userdata_cell<T: 'static + UserData>(state: *mut ffi::lua_State) -> UserDataCell<T> {
	let userdata = &*get_userdata::<UserDataCell<T>>(state, -1);
	userdata.clone()
}
