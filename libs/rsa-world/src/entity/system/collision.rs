		...
		for (_, (collision, position, physics)) in storage.query_mut::<(
			...
			for (pos, _) in &mut collision.collisions {
				if let Some(Some((d, contact))) =
					resolve_dynamic_rect_vs_rect(physics.vel, old_rect, 1.0, *pos)
				{
					draw_debug!(
						debug,
						DebugCategory::EntityCollision,
					...
					if let Some(callback) = &collision.hit_callback {
						...
						let contact = core.lua.to_value(&contact).unwrap();
...
fn test_collision(
	...
	if let Some((pos, contact_time)) = dynamic_rect_vs_rect(vel, collision_area, 1.0, rect)
