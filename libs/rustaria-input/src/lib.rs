...
impl InputSystem {
	...
	pub fn notify_event(&mut self, action: Event) {
		if action.kind.sustained() {
			if action.pressed {
				...
				self.active.insert(action.kind.clone(), (action.clone(), EventState  {
					fire_press: true,
					fire_release: false
				}));
			} else {
				...
				if let Some((press_action, mut state)) = self.active.remove(&action.kind) {
					...
					self.subtick_history.push(EventRecord::new(action, EventState::full()));
				}
			}
		} else {
			self.subtick_history.push(EventRecord::new(action, EventState::full()));
	...
	pub fn tick(&mut self) {
		...
		for action in &mut self.subtick_history {
			...
			Self::invoke_subs(&self.bindings, &mut self.subscribers, &action.kind, &mut action.state, delta);
		}

		self.subtick_history.clear();
	}

	fn invoke_subs(bindings: &HashMap<EventKind, Vec<Tag>>, subscribers: &mut HashMap<Tag, Vec<Subscriber>>, kind: &EventKind, state: &mut EventState, delta: f32) {
...
mod tests {
	...
	fn test_trigger_subscriber() {
		...
		let mut input = InputSystem::new();
		input.register_binding(Tag::builtin("test"), vec![EventKind::Scroll(Direction::Up)]);
		input.register_subscriber(Tag::builtin("test"), Subscriber::Trigger(Box::new(sub.clone())));
	...
	fn test_toggle_subscriber() {
		...
		let mut input = InputSystem::new();
		input.register_binding(Tag::builtin("test"), vec![EventKind::Scroll(Direction::Up)]);
		input.register_subscriber(Tag::builtin("test"), Subscriber::Toggle(Box::new(sub.clone()), false));
	...
	fn test_hold_subscriber() {
		...
		let mut input = InputSystem::new();
		input.register_binding(Tag::builtin("test"), vec![EventKind::Keyboard(KeyboardEvent {
			key: Key::Space,
			modifier: Modifiers::empty()
		})]);
		input.register_subscriber(Tag::builtin("test"), Subscriber::Hold(Box::new(sub.clone())));
