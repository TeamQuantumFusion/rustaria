...
impl<T: Clone> GenerationSettings<T> {
	...
	pub fn sort(&mut self) {
		self.zones.sort_by(|(_, v0), (_, v1)| v0.priority.total_cmp(&v1.priority));
	...
	pub fn zone_lookup(&self) -> HashMap<Tag, ZoneId> {
		self.zones.iter().enumerate().map(|(id, (tag, _))| (tag.clone(), ZoneId(id as u16))).collect()
	}

	pub fn climate_lookup(&self) -> HashMap<Tag, ClimateId> {
		self.climates.iter().enumerate().map(|(id, (tag, _))| (tag.clone(), ClimateId(id as u16))).collect()
	}
	pub fn biome_lookup(&self) -> HashMap<Tag, BiomeId> {
		self.biomes.iter().enumerate().map(|(id, (tag, _))| (tag.clone(), BiomeId(id as u16))).collect()
	}
}
...
impl BiomeProducer {
	pub fn new(settings: BiomeProducerSettings, biomes_lookup: &HashMap<Tag, BiomeId>) -> BiomeProducer {
		BiomeProducer {
