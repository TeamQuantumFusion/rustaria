use ahash::AHashMap;
use rsa_hash::Hasher;
use crate::{Id, Identifier, IdValue, Storage};

#[derive(Clone, Debug)]
pub struct RegistryLookup<V: IdValue> {
	pub(crate) id_to_identifier: Storage<Identifier, V>,
	pub(crate) identifier_to_id: AHashMap<Identifier, Id<V>>,
}

impl<V: IdValue> RegistryLookup<V> {
	...
	pub fn map<O: IdValue>(self) -> RegistryLookup<O> {
		RegistryLookup {
...
impl<V: IdValue> Default for RegistryLookup<V> {
	fn default() -> Self {
		RegistryLookup {
