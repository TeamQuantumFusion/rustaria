...
use rectangle_pack::{contains_smallest_box, pack_rects, volume_heuristic, GroupedRectsToPlace, RectToInsert, RectanglePackError, TargetBin, PackedLocation};
...
pub fn build_atlas(api: &Api, sprites: HashSet<Tag>) -> (Atlas, Vec<(DynamicImage, AtlasLocation)>) {
    ...
    // Try to insert, if it does not have enough space double the atlas size.
