...
impl PluginLoader {
	pub async fn scan_and_load_plugins<'lua>(&self, lua: &'lua Lua) -> eyre::Result<Plugins<'lua>> {
	...
	pub async fn load_plugin<'lua>(&self, path: &Path, lua: &'lua Lua) -> eyre::Result<Plugin<'lua>> {
