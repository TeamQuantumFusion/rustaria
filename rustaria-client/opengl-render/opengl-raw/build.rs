...
impl Generator for NiceGenerator {
    fn write<W>(&self, registry: &Registry, dest: &mut W) -> std::io::Result<()> where W: Write {
        // Header
        dest.write_all("#![allow(non_upper_case_globals, non_snake_case, non_camel_case_types, dead_code)]\n".as_ref())?;
        ...
        dest.write_all(String::from_utf8(write).unwrap().replace("super::__gl_imports::raw::", "std::os::raw::").as_ref())?;
        ...
        dest.write_all("pub unsafe fn load_gl<F: FnMut(&'static str) -> *const c_void>(func: &mut F) {".as_ref())?;
        for x in &registry.cmds {
            ...
            dest.write_all(format!("_{indent} = transmute(load(func, \"{indent}\", {fallbacks}));\n").as_ref())?;
        }
        dest.write_all("}".as_ref())?;
...
pub fn write_fn_storage<W: io::Write>(w: &mut W, cmd: &Cmd) {
    ...
    let string = format!("static mut _{name}: extern \"system\" fn{fn_str} = unsafe {{ transmute(ERR) }};\n");
    w.write_all(string.as_ref());
}
...
pub fn write_const<W: io::Write>(w: &mut W, en: &Enum) {
    let string = format!("pub const {name}: {ty} = {value};\n",
                         name = en.ident,
                         ty = ty_str(&en.ty),
                         value = en.value
    );
...
fn ty_str(str: &Cow<'static, str>) -> String {
    str.replace("types::", "").replace("__gl_imports::raw::", "")
}
...
fn params_str(params: &Vec<Binding>, names: bool) -> String {
    let params: Vec<String> = params.iter().map(|bind| if names {
